...
namespace Fergun.Interactive
{
    internal static class InteractiveGuards
    {
        ...
        [Conditional("NO_DNETLABS")]
        public static void CanUseComponents<TOption>(IInteractiveElement<TOption> element)
        {
            if (element.InputType == InputType.Buttons
                || element.InputType == InputType.SelectMenus
                || element.ActionOnTimeout.HasFlag(ActionOnStop.DisableInput)
                || element.ActionOnCancellation.HasFlag(ActionOnStop.DeleteInput))
            {
